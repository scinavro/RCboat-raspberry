...
def check_if_touched(x_acc, y_acc, z_acc, prev_acc, prev_z):
	cur_acc = (x_acc**2 + y_acc**2 + z_acc**2)**(1/2)
	if((cur_acc > prev_acc*1.4 or cur_acc < prev_acc*0.6) and (z_acc > prev_z*1.4 or z_acc < prev_z*0.6)):
		return True
	return False
...
while True:
    ...
    data = 'isDetected = True' + '37.613883, 126.384635' 
    s.send(data.encode())

    print (" Reading the initial data of Accelerometer")
    f_acc_x = read_raw_data(ACCEL_XOUT_H)
    f_acc_y = read_raw_data(ACCEL_YOUT_H)
    f_acc_z = read_raw_data(ACCEL_ZOUT_H)
    f_Ax = f_acc_x/16384.0
    f_Ay = f_acc_y/16384.0
    f_Az = f_acc_z/16384.0
    prev_acc = (f_Ax**2 + f_Ay**2 + f_Az**2)**(1/2)
    prev_z = f_Az
    ...
    while(isDetected):
        ...
        if(check_if_touched(Ax, Ay, Az, prev_acc, prev_z) and isTouched == False):
            print("====Boat is touched====")
            data = 'isTouched = True'  + '37.613883, 126.384635'  
            s.send(data.encode())
            while True:
                stop()

        isTouched = check_if_touched(Ax, Ay, Az, prev_acc, prev_z)
        prev_acc = (Ax**2 + Ay**2 + Az**2)**(1/2)
        prev_z = Az
        time.sleep(0.5)
